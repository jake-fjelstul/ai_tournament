...
class RatBelief:
    def __init__(self, T):
        ...
        # PRECOMPUTE the 1000-step headstart matrix ONCE
        self.T_1000 = np.linalg.matrix_power(self.Tt, 1000)
        self.belief = np.zeros(64)

    def initialize(self):
        v = np.zeros(64)
        v[0] = 1.0  # rat placed at (0,0)
        # Instantly apply the 1000-step jump without a loop
        v = self.T_1000 @ v
        self.belief = v / v.sum()
    ...
    def update_distance(self, reported_d, worker_pos):
        ...
        for i in range(64):
            ...
            for raw_offset, prob in DIST_OFFSET.items():
                reported = max(0, true_d + raw_offset)
                if reported == reported_d:
                    p += prob
            likelihoods[i] = p
